####
# This code was originally implemented by Seth Ebner, a few minor edits were applied to make it work with better data
####
...
MY_MODEL="/srv/local1/shabnam/projects/EnFa-large-128K-v1.0"
MY_HD=1024
def parse_args():
...
def coherent(subtoken_embeddings, subtoken_span_boundaries):
	endpoint_representation = EndpointSpanExtractor(MY_HD)._embed_spans(subtoken_embeddings, torch.as_tensor(subtoken_span_boundaries).unsqueeze(0))[0]
	...
	if MY_HD==1024:
		a=480
		b=32
	left_endpoint = endpoint_representation[:, :MY_HD]
	right_endpoint = endpoint_representation[:, MY_HD:]
...
def align(source_tokens, target_tokens, tokenizer, model, model_layer, span_extractor, decoding_method, source_span_boundaries=None, target_span_boundaries=None, source_lang="en", target_lang="fr", max_source_span_width=None, max_target_span_width=None, allow_overlap=False, couple=False, verbose=False, span_ids=None,event_ids=None, ent_names=None):
	...
	if decoding_method == "greedy":
		...
		for _ in range(table.size):
			...
			s_id=span_ids[source_span_idx]
			e_id=event_ids[source_span_idx]
			e_name= ent_names[source_span_idx]
			...
			if allow_overlap or (not overlaps(source_span[0], source_span[1], used_source_spans) and not overlaps(target_span[0], target_span[1], used_target_spans)):
				...
				result.append({"s_id":s_id,"e_id":e_id,"e_name":e_name,"source_span": source_span, "target_span": target_span, "source_span_tokens": source_span_tokens, "target_span_tokens": target_span_tokens, "score": table[best_alignment]})
		...
		for alignment in alignments:
			...
			if allow_overlap or (not overlaps(source_span[0], source_span[1], used_source_spans) and not overlaps(target_span[0], target_span[1], used_target_spans)):
				...
				result.append({"s_id":s_id,"e_id":e_id,"e_name":e_name,"source_span": source_span, "target_span": target_span, "source_span_tokens": source_span_tokens, "target_span_tokens": target_span_tokens, "score": table[alignment]})
...
def main():
	...
	model = AutoModel.from_pretrained(MY_MODEL, output_hidden_states=True)
	...
	if args.span_extractor == "mean_pool":
		...
		span_extractor = EndpointSpanExtractor(MY_HD)
	elif args.span_extractor == "diffsum":
		# https://aclanthology.org/2020.repl4nlp-1.20/
		span_extractor = EndpointSpanExtractor(MY_HD, combination="x+y,y-x")
